...
function newGameState() {
  return {
    ...
    config: {
      ...
      apiKey:    'sk-vOHDPKxcBFCsETCnZ3uRu9lsW67X1mjr',
...
function buildSystemPrompt() {
  ...
  return `# 命运织机 — AI DM 指令

## 角色
你是 AI Dungeon Master，负责：
1. 编织剧情 — 根据玩家行动生成剧情
2. 扮演 NPC — 对话、表情、动作
3. 推进世界 — NPC 有动机，事件自然发生
4. 管理游戏 — 判定结果，更新状态
...
## 角色信息
姓名: ${c.name} | ${c.race} ${c.cls} | ${c.origin}
属性: { ${statsStr} }
等级: ${gameState.level} | HP: ${gameState.hp}/${gameState.maxHp} | MP: ${gameState.mp}/${gameState.maxMp} | 金币: ${gameState.gold}
技能: ${clsInfo.skills ? clsInfo.skills.join('、') : '无'}

## 输出格式
回复必须包含以下部分（按顺序）：

**【剧情】** 剧情段落（80-150字），包含环境描写和NPC对话。**简洁，不啰嗦，少用修饰词，直接推进剧情。**
...
**【建议】** 3-5个玩家下一步可以做的行动，每行一个编号，如：
1. 推开门走进去
2. 问守卫更多信息
3. 观察周围环境

## 规则
- ✏️ **回答精简**：场景描写 ≤ 100 字，NPC 对话简短，减少修饰词
- 🚫 不要替玩家做决定，不要写"你选择去"
- ✅ 战斗有紧张感，不自动成功
- ✅ 保持世界一致性
- ✅ 中文回复，沉浸感`;
...
async function processAIResponse(content) {
  ...
  // 提取【建议】选项
  let suggestions = [];
  const sugMatch = cleanContent.match(/【建议】\s*([\s\S]*)/);
  if (sugMatch) {
    suggestions = sugMatch[1]
      ...
      .filter(s => s.length > 0);
    cleanContent = cleanContent.replace(/【建议】[\s\S]*/, '').trim();
  }
  ...
  // 更新推荐选项
  updateQuickActions(suggestions.length > 0 ? suggestions : null);
...
function updateQuickActions(suggestions) {
  ...
  if (suggestions && suggestions.length > 0) {
    // 显示 AI 推荐选项
    actions = suggestions.map(s => ({ text: s, action: s }));
  } else {
    // 默认快捷操作
    actions = [
      { text: '🗺️ 查看周围', action: '描述我现在周围的环境' },
      { text: '👀 观察', action: '仔细观察周围，寻找线索' },
      { text: '💬 对话', action: '向附近的NPC打招呼，尝试交谈' },
      { text: '🔍 搜索', action: '搜索这个地方，寻找隐藏物品' },
      { text: '🚶 离开', action: '离开当前地点，探索新的区域' },
    ];
  }

  el.innerHTML = actions.map(a =>
    `<button class="quick-btn" onclick="handleQuickAction('${escHtml(a.action)}')">${escHtml(a.text)}</button>`
  ).join('');
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // 随机分配
  document.getElementById('randomize-stats').addEventListener('click', window.randomizeStats);
